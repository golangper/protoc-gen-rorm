...
func (p *TsPlugin) Generate(file *generator.FileDescriptor) {
	...
	p.P(`import { HttpClient, HttpHeaders,HttpParams } from '@angular/common/http';`)
				...
				if api.GetMethod() == `get` || api.GetMethod() == `GET` || api.GetMethod() == `Get` {
					if len(msg.Field) != 0 {
						...
						for i, f := range msg.Field {
							...
							lb := f.GetLabel()
							tp := f.GetType()
							if descriptor.FieldDescriptorProto_TYPE_MESSAGE == tp  || lb == descriptor.FieldDescriptorProto_LABEL_REPEATED {
								continue
							}
							
							if i < len(msg.Field)-1 {
								p.P(`.set('`, fname, `', '' + param.`, fname, `)`)
							} else {
								p.P(`.set('`, fname, `', '' + param.`, fname, `)`)
							}
